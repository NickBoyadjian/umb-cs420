 ...
 *)
Theorem ex1: Decidable (fun w => True).
Proof.
  apply p_decidable_def with (p := fun _ => ACCEPT).
  apply p_decides_def; intros.
  - apply p_recognizes_def.
    { intros. apply I. } { intros. apply run_ret. }
  - intros.
    apply p_decider_def.
    intros.
 ...
 *)
Theorem ex2: PRecognizes (fun w=>LOOP) (fun w =>False).
Proof.
  apply p_recognizes_def.
  - intros. inversion H.
  - intros. inversion H.
 ...
 *)
Theorem ex3: forall f, Decidable (fun i => f i = true).
Proof.
  intros.
  unfold Decidable.
 ...
 *)
Theorem ex4: forall p, Run (Seq LOOP p) Loop.
Proof.
  intros.
  apply run_seq_loop.
  apply run_ret.
 ...
 *)
Theorem ex5: forall p, Run (Seq p (fun b => LOOP)) Loop.
Proof.
  intros. 
...
Hard. If a given program decides a language, then negating its result recognizes 
the complement of that language. Make sure you use p_decides_run_reject and 
p_decides_reject. In the second branch of proving recognizability, 
make sure you do a case analysis on the result of running p with i, 
that is to say, use:
 ...
 *)
Theorem ex6_1: forall p L, PDecides p L -> PRecognizes (fun i =>
...
Proof.
  ...
  - inversion H0. subst.
    ...
    { apply H. } 
    { run_simpl_all. subst. run_simpl_all. apply H3. }
  - destruct (run_exists (p i)) as (r, Hr).
    apply run_seq_reject.
 ...
 *)
Theorem ex6_2: forall p, PDecider p -> PDecider (fun i : input => mlet b <- p i in halt_with (negb b)).
Proof.
  intros.
  apply p_decider_def.
  intros.
 ...
 *)
Theorem ex6_3: forall p L, PDecides p L -> PDecides (fun i =>
    mlet b <- p i in halt_with (negb b)) (compl L).
Proof.
  intros. 
  apply p_decides_def.
 ...
 *)
Theorem ex6: forall L, Decidable L -> Decidable (compl L).
Proof.
  intros.
