 ...
 *)
Theorem E_tm_red_EQ_tm_1: forall w, E_tm w -> EQ_tm (F3 w).
Proof.
Admitted.
 ...
 *)
Theorem E_tm_red_EQ_tm_2: forall w, EQ_tm (F3 w) -> E_tm w.
Proof.
Admitted.

(** Easy. Solve Exercise 5.26 of the book (pp 237). *)
Theorem E_tm_red_EQ_tm: E_tm <=m EQ_tm.
Proof.
  split with (x:=F3).
  split.
...
Theorem thm_5_4: ~ Decidable EQ_tm.
Proof.
...
(**
    Easy.
    Solve Exercise 5.6 (pp 239; solution in pp 242).
    You should use `reducible_def` and then unfold Reduction.
 *)
Theorem ex_5_6:
    forall A B C,
    A <=m B ->
...
(** Solve Exercise 5.7 (pp 230; solution in pp 242). *)
Theorem ex_5_7:
    forall A,
    Recognizable A ->
    A <=m compl A ->
...
Theorem ex_5_22_b: forall A, A <=m A_tm -> Recognizable A.
Proof.
  intros.
 ...
 *)
Theorem ex_5_22: forall A, Recognizable A <-> A <=m A_tm.
Proof.
  split.
  - apply ex_5_22_a.
  - apply ex_5_22_b.
...
(** Hard. Easy to solve if done in pen-and-paper first. Some results needed to prove Exercise 5.22 are helpful here.  *)
